...
class PPINs(keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = tf.reduce_mean(tf.reduce_mean(tf.square(y['u_data'] - y_pred[:, 0]))) + tf.reduce_mean(tf.square(y['v_data'] - y_pred[:, 1])) + PPINs.pred(y_pred, x, self.a, self.b, self.c, self.d, self.normalized_data_list)
        ...
        return {"loss": loss_tracker.result(), "mae": mae_metric.result(), 'a': self.a, "b": self.b, 'c': self.c,'d': self.d}
    ...
    def pred(y_pred, x, a, b, c, d, normalized_data_list: NormalizedDataList):
        ...
        loss1 = tf.reduce_mean(tf.square(v_t - c * u * v + d * v))
        loss2 = tf.reduce_mean(tf.square(u_t - a * u + b * u * v))
        return loss1 + loss2
